...
public class GliderLayer extends RenderLayer<AbstractClientPlayer, PlayerModel<AbstractClientPlayer>> {
    private static final Map<Item, ResourceLocation> TEXTURE_LOCATIONS = ImmutableMap.<Item, ResourceLocation>builder().put(ModRegistry.HANG_GLIDER_ITEM.get(), HangGlider.id("textures/models/glider/hang_glider.png")).put(ModRegistry.REINFORCED_HANG_GLIDER_ITEM.get(), HangGlider.id("textures/models/glider/reinforced_hang_glider.png")).build();
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, AbstractClientPlayer player, float limbSwing, float limbSwingAmount, float partialTick, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        ItemStack stack = PlayerGlidingHelper.isGliderDeployed(player) ? PlayerGlidingHelper.getGliderInHand(player) : ItemStack.EMPTY;
